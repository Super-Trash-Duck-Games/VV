...
public class CRunState : State
{
    ...
    {
        if (_cientist.transform.position.x > _data.middle.position.x + _data.targetTolerance)
        {
            _cientist.transform.localScale = new Vector2(-1, 1);

            _data.rb2d.AddForce(Vector2.right * _data.runSpeed * 100 * Time.deltaTime * -1, ForceMode2D.Force);
        }
        else if (_cientist.transform.position.x < _data.middle.position.x - _data.targetTolerance)
        {
